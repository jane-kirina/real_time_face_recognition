...
def test_log_detection_calls_write_event(monkeypatch):
    ...
    logger.log_detection('alice', face_id=1, confidence=0.9)

    assert len(called) == 1
    assert called[0]['event'] == 'PERSON_DETECTED'
    assert called[0]['name'] == 'alice'
    assert called[0]['confidence'] == 0.9
...
def test_log_detection_unknown_event(monkeypatch):
    ...
    logger.log_detection('unknown', face_id=1, confidence=0.3)

    assert len(called) == 1
    assert called[0]['event'] == 'UNKNOWN_DETECTED'
...
def test_log_detection_respects_cooldown(monkeypatch):
    ...
    logger.log_detection('alice', face_id=1, confidence=0.9)
    logger.log_detection('alice', face_id=1, confidence=0.95)
...
def test_log_detection_uses_face_id_as_identity(monkeypatch):
    ...
    logger.log_detection('alice', face_id=1, confidence=0.9)
    logger.log_detection('alice', face_id=2, confidence=0.9)
...
def test_log_detection_uses_name_if_no_face_id(monkeypatch):
    ...
    logger.log_detection('alice', face_id=None, confidence=0.9)
    logger.log_detection('alice', face_id=None, confidence=0.95)
